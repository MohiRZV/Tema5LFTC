...
vector<pair<string, string>> fip;

string readFIP() {
	ifstream fin("source_fip.txt");
	...
	while (fin >> atomType) {
		fin >> value;
		sequence.append(atomType);
		fip.push_back(make_pair(atomType, value));
	}
	return sequence;
}
...
int main(int argc, char const *argv[])
{
	...
	// Get input sequence
	string input_string = sequence;// (argv[2]);
	cout << "\n\nEntered a squence:\n"<<sequence<<'\n';
